...
import type { LeaderboardPowertrain, VehicleView } from "../lib/types";

interface Props {
  view: DashboardView;
  vehicleView: VehicleView;
  setVehicleView: (value: VehicleView) => void;
  leaderboardPowertrain: LeaderboardPowertrain;
  setLeaderboardPowertrain: (value: LeaderboardPowertrain) => void;
  activeFleetAge: number | null;
  setActiveFleetAge: (value: number | null) => void;
          ...
          <article className="panel powertrain-panel">
            <div className="panel-heading">
              <div>
                ...
                <h3>Combustion still leads.<br />Electrified cars are visible.</h3>
              </div>
              <strong>{percent(view.electric / view.vehicleTotal)}<small>BEV + PHEV</small></strong>
            </div>
                ...
                <div className="bar-row" key={row.name}>
                  <div>
                    ...
                    <span className="bar-value">
                      ...
                      {vehicleView === "latest" && (
                        <MonthChangeIndicator change={view.powertrainChanges.get(row.name) ?? null} previousMonth={view.previousMonth} />
                      )}
                    </span>
                  </div>
              ...
              {view.arrivalMix.map((row) => (
                <div className="arrival-mix-row" key={row.name}>
                  <div>
                    ...
                    <small>{percent(row.nzNew / row.total)} new · {percent(row.used / row.total)} used</small>
              ...
              {view.topModels.map((row) => (
                <li key={`${row.make}-${row.model}`}>
                  ...
                  <span className="ranking-value">
                    <b>{number.format(row.registration_count)}</b>
                    {vehicleView === "latest" && (
                      <MonthChangeIndicator change={view.modelChanges.get(`${row.make}\u0000${row.model}`) ?? null} previousMonth={view.previousMonth} />
                    )}
                  </span>
                </li>
              ))}
        ...
        <article className="panel fleet-age-panel">
          <div className="fleet-age-heading">
            ...
            <div className="fleet-age-stats">
              <span><b>{view.fleetAgeMean.toFixed(1)}</b>average years</span>
              <span><b>{view.fleetAgeMedian}</b>median years</span>
              <span><b>{view.fleetAgeMode.approximate_current_age}</b>most common age</span>
            </div>
          </div>
          ...
          <div className="fleet-age-chart" aria-label="Current registered passenger fleet by approximate age">
            {view.fleetAgeBuckets.map((row) => (
              ...
              >
                <i style={{ height: `${Math.max(2, (row.value / view.fleetAgeMax) * 100)}%` }} />
                <span>{row.age % 5 === 0 || row.age === 31 ? row.label : ""}</span>
              </button>
            ))}
          </div>
          <p className="chart-note">Approximate whole-year age = {view.latest.slice(0, 4)} snapshot year minus vehicle year. Includes the full ordinary passenger-vehicle snapshot with a usable vehicle year; 31+ is grouped for readability. Vehicle-year meanings before {view.startYear} are less consistent.</p>
        </article>
      </div>
    </section>
  );
