import type { DashboardView, Range } from "../types";
import { number, percent, prettyMonth, prettyMonthName } from "../utils/formatters";
...
export function MarketFlowSection({
  ...
  activeAnnual: { year: number; nz_new: number; used_import: number } | undefined;
  setActiveMarketYear: (year: number) => void;
  selectMarketYear: (year: number) => void;
  marketMonth: string;
  marketMonthIndex: number;
  marketMonths: string[];
  marketYears: string[];
  marketYearMonths: string[];
  setSelectedMarketMonth: (month: string) => void;
  monthlyDetail: { nzNew: number; used: number; other: number };
    ...
    <section className="section wrap" id="market">
      ...
      <div className="chart-card market-card">
        <div className="market-chart-toolbar">
          {activeAnnual && (
            <div className="annual-readout" aria-label={`Selected year ${activeAnnual.year}`}>
              ...
              <span className="readout-new">
                <b>{number.format(activeAnnual.nz_new)}</b>NZ-new
              </span>
              <span className="readout-used">
                <b>{number.format(activeAnnual.used_import)}</b>Used imports
              </span>
            </div>
          )}
          <div className="chart-key" aria-hidden="true">
            <span className="key-new">NZ-new</span>
            <span className="key-used">Used imports</span>
          </div>
        </div>
        <div
          className="year-chart"
          role="group"
          aria-label="Annual NZ-new and used-import passenger vehicle registrations"
        >
          {view.annual.map((row) => (
            <button
              type="button"
        ...
        <p className="chart-note">
          {view.latest.slice(0, 4)} is year-to-date through {prettyMonthName(view.latest)}. Hover or focus for annual
          counts; select a year to open its latest month below.
        </p>
      </div>
      ...
      <div className="monthly-detail-heading">
        ...
        <div className="monthly-detail-controls" aria-label="Monthly detail period">
          ...
          <label>
            <span>Year</span>
            <select
              value={marketMonth.slice(0, 4)}
              onChange={(event) => selectMarketYear(Number(event.target.value))}
            >
      ...
      <div className="insight-strip">
        <article>
          <span>NZ-new</span>
          <strong>{number.format(monthlyDetail.nzNew)}</strong>
          <p>vehicles in {prettyMonth(marketMonth)}</p>
        </article>
        <article>
          <span>Used imports</span>
          <strong>{number.format(monthlyDetail.used)}</strong>
          <p>vehicles in {prettyMonth(marketMonth)}</p>
        </article>
        <article className="accent">
          <span>Import mix</span>
          <strong>{percent(monthlyDetailTotal ? monthlyDetail.used / monthlyDetailTotal : 0)}</strong>
          <p>of {prettyMonth(marketMonth)} entries came in used</p>
        </article>
      </div>
    </section>
  );
