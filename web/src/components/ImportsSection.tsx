import { compact, number, prettyMonth } from "../lib/utils";
...
export function ImportsSection({ view, countryView, setCountryView, topCountries, countryMax, countryViewLabel }: Props) {
  return (
    <section className="section wrap" id="imports">
      ...
      <div className="imports-grid">
        <article className="panel country-panel">
          ...
          <p className="chart-note">
            {countryView === "latest"
              ...
              : `Previous registration/import country among ${view.startYear}+ used imports still represented in the current fleet snapshot; not manufacturing country.`}
          </p>
        </article>
        ...
        <article className="panel age-panel">
          ...
          <p className="chart-note">Comparable cohort only: used imports with vehicle year {view.startYear} or later.</p>
        </article>
      </div>
    </section>
  );
